...
/****** Object:  StoredProcedure [dbo].[SPED_ArchivoTXT_ECF_l500]    Script Date: 26/09/2019 06:48:51 ******/
DROP PROCEDURE [dbo].[SPED_ArchivoTXT_ECF_l500]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[SPED_ArchivoTXT_ECF_l500] 
...
BEGIN
	...
	--print 'LLEGUE ' + rtrim(replace(convert(char,convert(datetime,@FechaHasta,102),103),'/',''))+'|'
	--------------------------------------------------------
	...
	INSERT INTO SPEDtbl9000 (LINEA,seccion, datos) 
		SELECT	@contador
				...
				,isnull('|0000|'+						--- REG,
						'LECF|'+								--- NOME_ESC,
						'0005|'+								--- COD_VER - MSAL(Verificar si esta ok)
	...
	OPEN Balance_cursor
	FETCH NEXT FROM Balance_cursor into @sped_cod_cta,@SPED_IND_CTA,@SPED_NIVEL,@SPED_COD_CTA_SUP,@SPED_COD_NAT,@ACTDESCR,@inicial,@tipoinicial,@credito,@debito,@final,@tipofinal
