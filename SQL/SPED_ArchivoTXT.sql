...
WHILE @@FETCH_STATUS = 0  
	begin
		...
		WHILE @@FETCH_STATUS = 0  
		begin
			exec SPED_Total_Saldo_Cta @actindx,@vanio,@vper,1,@debito out,@credito out,@tieneM output
			exec SPED_Total_Saldo_Cta @actindx,@vanio,@vper,2,@idebito out,@icredito out,@tieneI output
			exec SPED_Total_Saldo_Cta @actindx,@vanio,@vper,3,@fdebito out,@fcredito out,@tieneF output
...
DEALLOCATE periodos_cursor;
		...
		exec SPED_Total_Saldo_Cta @actindx,@vanio,12,1,@debitamt out,@crdtamnt out,@tieneF out
...
case SPED_NIVEL 
	...
	when 3 then (case when @RSped_Cod_Cta_N3 = g.SPED_COD_CTA then (select case when isnull(SUM(ABS(s.CRDTAMNT)),0)+@RMontod>isnull(SUM(ABS(s.DEBITAMT)),0)+@RMontoc then 'C' else 'D' end
	from GL10111 s
	left join GL00100 pc on s.ACTINDX = pc.actindx
	left join SPEDtbl004 n5 on n5.SPED_COD_CTA = pc.userdef1
	left join spedtbl004 n4 on n4.SPED_COD_CTA = n5.SPED_COD_CTA_SUP
	...
	left join spedtbl004 n1 on n1.SPED_COD_CTA = n2.SPED_COD_CTA_SUP
	where s.YEAR1 =@vanio and n3.SPED_COD_CTA=g.SPED_COD_CTA) end)
	when 4 then (case when @RSped_Cod_Cta_N4 = g.SPED_COD_CTA then (select case when isnull(SUM(ABS(s.CRDTAMNT)),0)+@RMontod>isnull(SUM(ABS(s.DEBITAMT)),0)+@RMontoc then 'C' else 'D' end
	from GL10111 s
	left join GL00100 pc on s.ACTINDX = pc.actindx
	left join SPEDtbl004 n5 on n5.SPED_COD_CTA = pc.userdef1
	left join spedtbl004 n4 on n4.SPED_COD_CTA = n5.SPED_COD_CTA_SUP
	...
	when 5 then (case when @RSped_Cod_Cta_N5 = g.SPED_COD_CTA then (select case when isnull(SUM(ABS(s.CRDTAMNT)),0)+@RMontod>isnull(SUM(ABS(s.DEBITAMT)),0)+@RMontoc then 'C' else 'D' end
	from GL10111 s
	left join GL00100 pc on s.ACTINDX = pc.actindx
	left join SPEDtbl004 n5 on n5.SPED_COD_CTA = pc.userdef1
	left join spedtbl004 n4 on n4.SPED_COD_CTA = n5.SPED_COD_CTA_SUP
