...
IF EXISTS (
  SELECT * 
   ...
   WHERE SPECIFIC_SCHEMA = 'dbo'
     AND SPECIFIC_NAME = 'SPED_ArchivoTXT_l800' 
)
   DROP PROCEDURE dbo.SPED_ArchivoTXT_l800;
GO
...
insert into spedtbl9000 (linea,seccion, datos)	
	values( @contador+5,'I010',isnull('|I010|'+	--- AS REG,
			'G|'+		--- AS IND_ESC,
			'8.00|'	,''))	--- AS COD_VER_LC						---SECCION I010
	...
	begin
		...
		insert into spedtbl9000 (linea,seccion,datos)
			select @contador+1,'I200',isnull('|I200|'+
				ltrim(rtrim(str(@JRNENTRY)))+'|'+                                               --NUM_LCTO
				rtrim(replace(convert(char,convert(datetime,@TRXDATE,103),103),'/',''))+'|'+    --DT_LCTO
				isnull(ltrim(rtrim(REPLACE(cast(convert(decimal(10,2),sum(@CRDTAMNT)) as nvarchar),'.',','))),'0,00') +'|'   --VL_LCTO
				+'N|'                                                                           --IND_LCTO
                +'|'                                                                            --DT_LCTO_EXT
				,'')
		set @contador=@contador+1
...
set @contador=@contador+1
		insert into spedtbl9000 (linea,seccion,datos)
			select @contador+1,'I200',isnull('|I200|'+
				rtrim(replace(convert(char,convert(datetime,@FechaHasta,102),103),'/',''))+'|'+ --NUM_LCTO
				rtrim(replace(convert(char,convert(datetime,@FechaHasta,102),103),'/',''))+'|'+ --DT_LCTO
				isnull(ltrim(rtrim(REPLACE(cast(convert(decimal(10,2),
                    abs(@resultado)
                        ) as nvarchar),'.',','))),'0,00')                                   --VL_LCTO
				+'|E|'                                                                      --IND_LCTO
                +'|'                                                                        --DT_LCTO_EXT
                ,'')
set @contador=@contador+1
	...
	BEGIN
		...
		insert into spedtbl9000 (linea,seccion,datos)
			select @contador+1,'J100',isnull('|J100|'+
                ...
                isnull(ltrim(rtrim(REPLACE(cast(convert(decimal(18,2),@Saldo) as nvarchar),'.',','))),'0,00')+'|'+      --VL_CTA_FIN
				@TIPO+'|'+															--IND_DC_CTA_FIN
                '|'																	--NOTA_EXP_REF
				,'|')
	END
...
go
-----------------------------------------------------------------------
IF (@@Error = 0) PRINT 'Creación exitosa de: SPED_ArchivoTXT_l800'
ELSE PRINT 'Error en la creación de: SPED_ArchivoTXT_l800'
GO
